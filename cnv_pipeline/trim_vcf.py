...
def trim_vcf(vcf_in=None, tumor_id=None, normal_id=None,
             min_ratio=0.4, max_ratio=0.6, min_depth_n=10, min_depth_t=20,
    ...
    cmd = (
        ...
        """&& 1.0 * vc.getGenotype("{normal}").getAD().1 /  vc.getGenotype("{normal}").getDP() > {min_ratio} """
        """&&  1.0 * vc.getGenotype("{normal}").getAD().1 /  vc.getGenotype("{normal}").getDP() < {max_ratio} """
